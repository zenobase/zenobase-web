(() => {
	...
	app.factory('SleepCycle', [
		...
		(moment) => {
			...
			function parseTimeInBed(value) {
				...
				if (m) {
					return parseInt(m[1], 10) * 60 * 60 * 1000 + parseInt(m[2], 10) * 60 * 1000;
				}
	...
	app.factory('Nomie', [
		...
		(moment) => {
			...
			return {
				parse: (s, settings) => (s.charAt(0) === '{' ? parseJSON(s, settings) : parseCSV(s, settings)),
			};
		},
	]);
	...
	app.factory('Nomie5', [
		...
		(moment) => {
			...
			return {
				parse: (s, settings) => parseCSV(s, settings),
			};
		},
	]);
	...
	app.controller('ImportDialogController', [
		'$scope',
		'$http',
		'$routeParams',
		'EventSpreadsheet',
		...
		'tracker',
		'delay',
		($scope, $http, $routeParams, EventSpreadsheet, HealthKit, SleepCycle, SleepyHead, SunSprite, TapLog, MoodPanda, Nomie, Nomie5, Basis, HabitBull, LibreView, tracker, delay) => {
			...
			$scope.formats = [
				...
				},
				{
					...
					parse: (data, settings) => HealthKit.parse(data, settings),
				},
				{
					id: 'libreview',
					label: 'LibreView',
					...
					parse: (data, settings) => LibreView.parse(data, settings),
				},
				{
					id: 'nomie',
					label: 'Nomie',
					...
					parse: (data, settings) => Nomie.parse(data, settings),
				},
				{
					id: 'nomie5',
					label: 'Nomie 5',
					...
					settings: '/import-nomie.html',
					parse: (data, settings) => Nomie5.parse(data, settings),
				},
				{
					id: 'moodpanda',
					label: 'MoodPanda',
				...
				},
				{
					...
					parse: (data, settings) => SleepyHead.parse(data, settings),
				},
				{
					id: 'sunsprite',
					label: 'SunSprite',
					...
					parse: (data, settings) => SunSprite.parse(data, settings),
				},
				{
					id: 'taplog',
					label: 'TapLog',
					...
					parse: (data, settings) => TapLog.parse(data, settings),
				},
			];
			...
			$scope.isEmpty = () => !$scope.events || $scope.events.length === 0;
			$scope.prev = () => {
				--$scope.offset;
			};
			$scope.next = () => {
			...
			$scope.submit = () => {
				...
				$http
					.post('/buckets/' + $scope.bucketId + '/', { events: $scope.events })
	...
	app.controller('ImportTapLogController', [
		'$scope',
		'Field',
		($scope, Field) => {
	...
	app.controller('ImportSunSpriteController', [
		'$scope',
		($scope) => {
	...
	app.controller('ImportLibreViewController', [
		'$scope',
		($scope) => {
			$scope.settings.timezone = 'UTC';
	...
	app.controller('ImportNomieController', [
		'$scope',
		'Field',
		($scope, Field) => {
			$scope.fields = Field.findByType('numeric');
	...
	app.controller('ImportSleepyHeadController', [
		'$scope',
		($scope) => {
	...
	app.controller('ImportMoodPandaController', [
		'$scope',
		($scope) => {
	...
	app.controller('ExportDialogController', [
		'$scope',
		'$routeParams',
		'$window',
		'token',
		'tracker',
		($scope, $routeParams, $window, token, tracker) => {
			...
			$scope.file = () => $scope.bucketId + '.' + $scope.media;
			$scope.submit = () => {
				$scope.alert.clear();
				$scope.closeDialog();
