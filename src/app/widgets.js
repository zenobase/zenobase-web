(() => {
	...
	app.factory('WidgetControllerSupport', () => ($scope) => {
		$scope.classesForOrderBy = (column) => {
			var classes = [];
	...
	app.factory('WidgetDialogControllerSupport', [
		'Field',
		(Field) => ($scope, orderBy) => {
			$scope.init = () => {
			...
			$scope.getField = (name) => Field.find(name);
			if (orderBy) {
				...
				$scope.order = (order) => {
					...
					return $scope.settings.order && $scope.settings.order.charAt(0) === '-' ? $scope.settings.order.substr(1) : $scope.settings.order;
				};
			}
		},
	]);

	app.factory('WidgetFilter', [
		'Constraint',
		(Constraint) => {
	...
	app.controller('ListWidgetController', [
		'$scope',
		'WidgetFilter',
		($scope, WidgetFilter) => {
			$scope.init = () => {
			...
			$scope.hasNext = () => $scope.offset + $scope.settings.limit < $scope.total;
			$scope.prev = () => {
	...
	app.controller('ListWidgetDialogController', [
		'$scope',
		'WidgetDialogControllerSupport',
		'Field',
		($scope, WidgetDialogControllerSupport, Field) => {
			...
			WidgetDialogControllerSupport($scope, fields);
		},
	]);

	app.controller('CountWidgetController', [
		'$scope',
		'WidgetControllerSupport',
		($scope, WidgetControllerSupport) => {
			WidgetControllerSupport($scope);
	...
	app.controller('CountWidgetDialogController', [
		'$scope',
		'WidgetDialogControllerSupport',
		'Field',
		($scope, WidgetDialogControllerSupport, Field) => {
			WidgetDialogControllerSupport($scope, ['term', 'count']);

			$scope.fields = Field.findByType('text');
		},
	]);

	app.controller('GanttWidgetController', [
		'$scope',
		'WidgetControllerSupport',
		'timezone',
		($scope, WidgetControllerSupport, timezone) => {
			WidgetControllerSupport($scope);
	...
	app.controller('GanttWidgetDialogController', [
		'$scope',
		'WidgetDialogControllerSupport',
		'Field',
		($scope, WidgetDialogControllerSupport, Field) => {
	...
	app.controller('RatingsWidgetController', [
		'$scope',
		($scope) => {
			$scope.field = 'rating';
	...
	app.controller('RatingsWidgetDialogController', [
		'$scope',
		'WidgetDialogControllerSupport',
		($scope, WidgetDialogControllerSupport) => {
			WidgetDialogControllerSupport($scope);
		},
	]);

	app.controller('HistogramWidgetController', [
		'$scope',
		'$timeout',
		'Field',
		'Spreadsheet',
		($scope, $timeout, Field, Spreadsheet) => {
			$scope.init = () => {
			...
			$scope.draw = () => {
				if ($scope.intervals?.length) {
					var field = Field.find($scope.settings.field);
					var height = Math.max($scope.intervals.length * 20, 150);
					if ($scope.intervalsB?.length) {
						height *= 2;
					}
					var options = {
						chart: {
	...
	app.controller('HistogramWidgetDialogController', [
		'$scope',
		'WidgetDialogControllerSupport',
		'Field',
		($scope, WidgetDialogControllerSupport, Field) => {
			...
			$scope.valid = () => $scope.settings.interval > 0.0 && isUnitValid();
	...
	app.controller('ScoreboardWidgetController', [
		'$scope',
		'WidgetControllerSupport',
		($scope, WidgetControllerSupport) => {
			WidgetControllerSupport($scope);
			...
			$scope.selected = (value) => $scope.settings.statistics.indexOf(value) !== -1;
	...
	app.controller('ScoreboardWidgetDialogController', [
		'$scope',
		'WidgetDialogControllerSupport',
		'Field',
		($scope, WidgetDialogControllerSupport, Field) => {
			WidgetDialogControllerSupport($scope, ['term', 'count', 'sum', 'min', 'max', 'avg']);
			...
			$scope.$watch('settings.value_field', () => {
	...
	app.controller('TimelineWidgetController', [
		'$scope',
		'$timeout',
		'Field',
		'Interval',
		...
		'moment',
		'statistics',
		($scope, $timeout, Field, Interval, Spreadsheet, moment, statistics) => {
			$scope.keyField = 'timestamp';
			...
			$scope.update = (event, result, resultB) => {
				$scope.times = result[$scope.settings.id] || [];
				$scope.timesB = resultB?.[$scope.settings.id] || [];
			...
			$scope.draw = () => {
				if ($scope.times?.length || $scope.timesB?.length) {
	...
	app.controller('EffectSizeWidgetController', [
		'$scope',
		'$timeout',
		'Field',
		($scope, $timeout, Field) => {
			...
			$scope.update = (event, result, resultB) => {
				if ($scope.settings.statistic === 'avg') {
					...
					$scope.statsB = resultB?.[$scope.settings.id + '-stats'];
				} else {
			...
			function stats(times) {
				...
				var r = { avg: 0, stdev: 0, count: values.length },
					variance = 0;
				for (var m, s = 0, l = r.count; l--; s += values[l]);
				for (m = r.avg = s / r.count, l = r.count, s = 0; l--; s += (values[l] - m) ** 2);
	...
	app.controller('TimelineWidgetDialogController', [
		'$scope',
		'WidgetDialogControllerSupport',
		'Field',
		'Interval',
		($scope, WidgetDialogControllerSupport, Field, Interval) => {
			WidgetDialogControllerSupport($scope);
	...
	app.controller('PolarWidgetController', [
		'$scope',
		'$timeout',
		'Field',
		'Spreadsheet',
	...
	app.controller('PolarWidgetDialogController', [
		'$scope',
		'WidgetDialogControllerSupport',
		'Field',
		($scope, WidgetDialogControllerSupport, Field) => {
	...
	app.factory('statistics', () => {
		...
		return {
			regression: (data) => {
				...
				return !Number.isNaN(params.slope)
					? {
							data: [
	...
	app.controller('ScatterPlotWidgetController', [
		'$scope',
		'$timeout',
		'Field',
		'Spreadsheet',
		'timezone',
		'statistics',
		($scope, $timeout, Field, Spreadsheet, timezone, statistics) => {
			$scope.keyField = 'timestamp';
			...
			$scope.update = (event, result, resultB) => {
				...
				$scope.dataB = resultB?.[$scope.settings.id] || [];
				$timeout($scope.draw, 0); // delay for correct width
			};
			$scope.snapshot = () => {
				$scope.$broadcast('snapshot');
			...
			$scope.toSpreadsheet = () => {
				...
				var compareMode = $scope.dataB?.length;
			...
			$scope.draw = () => {
				...
				if ($scope.data?.length || $scope.dataB?.length) {
					var options = {
						chart: {
							type: 'scatter',
							zoomType: 'xy',
						...
						tooltip: {
							...
							formatter: function () {
								return (
									'<b>x</b>: ' +
									(xField.toText(this.x) || this.x) +
									($scope.settings.unit_x || '') +
									', ' +
									'<b>y</b>: ' +
									(yField.toText(this.y) || this.y) +
									($scope.settings.unit_y || '')
								);
							},
						},
						series: [
	...
	app.controller('ScatterPlotWidgetDialogController', [
		'$scope',
		'WidgetDialogControllerSupport',
		'Field',
		'Interval',
		($scope, WidgetDialogControllerSupport, Field, Interval) => {
			...
			$scope.valid = () => isUnitValid($scope.settings.field_x, $scope.settings.unit_x) && isUnitValid($scope.settings.field_y, $scope.settings.unit_y);
			$scope.swap = () => {
	...
	app.controller('MapWidgetController', [
		'$scope',
		'$timeout',
		($scope, $timeout) => {
			...
			$scope.update = (event, result, resultB) => {
				...
				$scope.boundsB = toBounds(resultB?.[$scope.settings.id] || {});
				$timeout($scope.draw, 1); // delay for correct width
			};
			...
			function getFilter() {
				...
				if ($scope.bounds && !$scope.bounds.isEmpty()) {
					...
					filter += 'location:' + [$scope.bounds.getSouthWest().lat(), $scope.bounds.getSouthWest().lng(), $scope.bounds.getNorthEast().lat(), $scope.bounds.getNorthEast().lng()].join(',');
				}
				return filter;
			}
			$scope.refreshPoints = () => {
				$scope.search([$scope.pointsParams()], (result, resultB) => {
					$scope.updatePoints(null, result, resultB);
				});
			};
			$scope.updatePoints = (event, result, resultB) => {
				$scope.points = result[$scope.settings.id] || [];
				$scope.pointsB = resultB?.[$scope.settings.id] || [];
				$scope.addPoints();
			};
			$scope.filterBounds = () => {
			...
			$('#' + $scope.settings.id + '-tab').on('shown', () => {
	...
	app.controller('MapWidgetDialogController', [
		'$scope',
		'WidgetDialogControllerSupport',
		($scope, WidgetDialogControllerSupport) => {
			WidgetDialogControllerSupport($scope);
	...
	app.controller('HeatmapWidgetController', [
		'$scope',
		'$timeout',
		'Field',
		($scope, $timeout, Field) => {
	...
	app.controller('HeatmapWidgetDialogController', [
		'$scope',
		'WidgetDialogControllerSupport',
		'Field',
		($scope, WidgetDialogControllerSupport, Field) => {
			...
			$scope.$watch('settings.value_field', (fieldName) => {
				var field = Field.find(fieldName);
				$scope.settings.unit = field?.units.length ? field.units[0] : '';
			});
		},
	]);

	app.controller('SonificationWidgetController', [
		'$scope',
		'$window',
		'$interval',
		($scope, $window, $interval) => {
			...
			$scope.isRunning = () => audio.state === 'running';
	...
	app.controller('SonificationWidgetDialogController', [
		'$scope',
		'WidgetDialogControllerSupport',
		($scope, WidgetDialogControllerSupport) => {
			WidgetDialogControllerSupport($scope);
