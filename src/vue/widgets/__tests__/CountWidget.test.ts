...
describe('CountWidget', () => {
	...
	it('mounts and registers with dashboard', () => {
		const { dashboard } = mountWidget(CountWidget, settings);
	...
	it('renders terms after update', async () => {
		const { wrapper, registration } = mountWidget(CountWidget, settings);
		await feedData(registration, 'w1', [{ label: 'sleep', count: 42 }]);
	...
	it('calls addConstraint when clicking a term', async () => {
		const { wrapper, dashboard, registration } = mountWidget(CountWidget, settings);
		await feedData(registration, 'w1', [{ label: 'sleep', count: 42 }]);
	...
	it('detects more pages when result exceeds limit', async () => {
		const { wrapper, registration } = mountWidget(CountWidget, { ...settings, limit: 2 });
		await feedData(registration, 'w1', [
			{ label: 'a', count: 3 },
			{ label: 'b', count: 2 },
			{ label: 'c', count: 1 },
